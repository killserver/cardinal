					...
					{% for (var i=0, file; file=o.files[i]; i++) { %}
					    <tr class="template-download fade">
					        ...
					        <td>
					            <p class="name">
					                {% if (file.url) { %}
					                    <a href="javascript:void('')" title="{%=file.name%}" class="link" data-file="{%=file.name%}" data-function="apply_img" {%=file.thumbnailUrl?'data-gallery':''%}>{%=file.name%}</a>
					                {% } else { %}
					                    <span>{%=file.name%}</span>
					                {% } %}
					            </p>
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			foreach ($files as $nu=>$file_array) {
				$file=$file_array['file'];
				if($file == '.' || $file == '..' || $file_array['extension']==fix_strtolower(trans('Type_dir')) || !check_extension($file_array['extension'],$config) || ($filter!='' && $n_files>$config['file_number_limit_js'] && stripos($file,$filter)===false) || hidden_mask($file, $config['hidden_mask']))
					continue;
