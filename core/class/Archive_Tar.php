...
class Archive_Tar extends PEAR {
    ...
    function addString($p_filename, $p_string, $p_datetime = false, $p_params = array()) {
        $p_stamp = isset($p_params["stamp"]) ? $p_params["stamp"] : ($p_datetime ? $p_datetime : time());
        $p_mode = isset($p_params["mode"]) ? $p_params["mode"] : 0600;
        $p_type = isset($p_params["type"]) ? $p_params["type"] : "";
        $p_uid = isset($p_params["uid"]) ? $p_params["uid"] : "";
        $p_gid = isset($p_params["gid"]) ? $p_params["gid"] : "";
        $v_result = true;
        if(!$this->_isArchive()) {
            if(!$this->_openWrite()) {
                return false;
    ...
    function _addString($p_filename, $p_string, $p_datetime = false, $p_params = array()) {
        ...
        $p_mode = isset($p_params["mode"]) ? $p_params["mode"] : 0600;
        $p_type = isset($p_params["type"]) ? $p_params["type"] : "";
        $p_uid = isset($p_params["uid"]) ? $p_params["uid"] : 0;
        $p_gid = isset($p_params["gid"]) ? $p_params["gid"] : 0;
        if(!$this->_file) {
            $this->_error('Invalid file descriptor');
            return false;
        }
