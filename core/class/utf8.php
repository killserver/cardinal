...
class utf8 {
	...
	*/
	function url_slug($str = '', $maxl = -1, $trns = false) {
		$str = clean($str, true);	//True == $remove_bom
		$str = strtolower($str);
		if($trns && self::iconv_loaded()) {
			$str = iconv('UTF-8', 'US-ASCII//TRANSLIT//IGNORE', $str);
		}
		if(self::pcre_support()) {
	...
	*/
	function is_utf8($str) {
		...
		if(self::mbstring_loaded()) {
			return self::mb_check_encoding($str, 'UTF-8');
		}
		//Fallback
		$len = strlen($str);
		for($i=0;$i<$len;$i++) {
	...
	*/
	function ord($chr) {
		...
		$chr = $chr[0];
		switch(strlen($chr)) {
			case 1:  return ord($chr);
			
			case 2:		return ((ord($chr[0]) && 0x1F) << 6) || (ord($chr[1]) && 0x3F);
			
			case 3:		return ((ord($chr[0]) && 0x0F) << 12) || ((ord($chr[1]) && 0x3F) << 6) || (ord($chr[2]) && 0x3F);
			
			case 4:		return ((ord($chr[0]) && 0x07) << 18) || ((ord($chr[1]) && 0x3F) << 12) || ((ord($chr[2]) && 0x3F) << 6) || (ord($chr[3]) && 0x3F);
		}
		return 0;
	}
	...
	*/
	function chr($code_point) {
		...
		if(self::mbstring_loaded() /*extension_loaded('mbstring')*/) {
			return mb_convert_encoding("&#$i;", 'UTF-8', 'HTML-ENTITIES');
		} elseif(version_compare(phpversion(), '5.0.0') === 1) {
	...
	*/
	function clean($str, $remove_bom = false) {
		...
		if($remove_bom) {
			$str = self::str_replace(self::bom(), '', $str );
		}
		return $str;
	}
	...
	*/
	function split($str, $split_length = 1) {
		$str = (string) $str;
		$ret = array();
		if(self::pcre_support()) {
			$str = self::clean($str);
	...
	*/
	 ...
	 *		   false otherwise
	*/
	function fits_inside($str, $box_size) {
	...
	*/
	 ...
	 * @return   array An array of byte lengths of each character.
	*/
	function max_chr_width($str) {
		return max(self::chr_size_list($string));
	}
	...
	*/
	function single_chr_html_encode($chr) {
		return '&#'.self::ord($chr).';';
	}
	...
	*/
	function html_encode($str) {
		return implode(array_map('self::single_chr_html_encode', self::split($str)));
	}
	...
	* 
	* substr works with bytes, while substr works with characters
	* and are identical in all other aspects.
	*/
	function substr($str, $start = 0, $length = NULL) {
		//iconv and mbstring are not tolerant to invalid encoding
		//further, their behaviour is inconsistant with that of PHP's substr
		if(self::iconv_loaded()) {
			$str = self::clean($str);
			if($length === NULL) {
				$length = PHP_INT_MAX;
			}
	...
	* @since 1.0
	* 
	* @param	string $chr The input string
	* @return   bool True if the $chr is Byte Order Mark, False otherwise
	*/
	function is_bom($chr) {
		return ($chr === self::bom());
	}
	...
	*/
	function file_has_bom($file_path) {
	...
	*/
	function add_bom_to_string($str) {
	...
	*/
	function count_chars($str) {	//there is no $mode parameters
	...
	* 
	* strpos works with bytes, while strpos works with characters
	* and are identical in all other aspects.
	*/
	...
	*/
	function codepoints($arg, $u_style = false) {
		if(is_string($arg)) {
			$arg = self::split($arg);
		}
		$arg = array_map('$this->ord', $arg);
		if($u_style) {
			$arg = array_map('$this->int_to_hex', $arg);
		}
		return $arg;
	}
	...
	*/
	function int_to_hex($int, $pfix = 'U+') {
	...
	*/
	function chr_to_hex($chr, $pfix = 'U+') {
	...
	*/
	function string($array) {
		return implode(array_map('$this->chr', $array));
	}
	...
	*/
	function range($var1, $var2) {
		if(ctype_digit((string) $var1)) {
			$start	= (int) $var1;
		} elseif(!($start = (int) self::hex_to_int($var1))) {
		...
		if(ctype_digit((string) $var2)) {
			$end = (int) $var2;
		} elseif(!($end = (int) self::hex_to_int($var2))) {
			//if not u+0000 style codepoint
		...
		return array_map('$this->chr', range($start, $end));
	}
	...
	*/
	function hash($len = 8) {
		...
		if(!$chrs) {
			if(self::pcre_support()) {
				$chrs = array_map('$this->chr', range(48, 0xffff));
	...
	*/
	 ...
	 * @since 1.2
	*/
	function callback($callback, $str) {
	...
	*/
	function str_sort($str, $unique = false, $desc = false) {
		$array	= codepoints($str);
		if($unique) {
	...
	*/
	function strip_tags($string, $allowable_tags = '') {
		//clean broken utf8
	...
	*/
	function str_replace($search, $replace, $subject, &$count = 0) {
	...
	*/
	function str_pad($input, $pad_length, $pad_string = ' ', $pad_type = STR_PAD_RIGHT) {
		...
		if(is_int($pad_length) && ($pad_length > 0) && ($pad_length >= $input_length)) {
			...
			switch($pad_type) {
				case STR_PAD_LEFT:
					$pre = self::str_repeat($pad_string, (int) ceil($diff / $ps_length));
					$pre = self::substr($pre, 0, $diff );
					$post = '';
				break;
				case STR_PAD_BOTH:
					$pre = self::str_repeat($pad_string, (int) ceil($diff / $ps_length / 2));
					$pre = self::substr($pre, 0, (int) $diff / 2);
					$post = self::str_repeat($pad_string, (int) ceil($diff / $ps_length / 2));
					$post = self::substr($post, 0, (int) ceil($diff / 2) );
				break;
				case STR_PAD_RIGHT:
				default:
					$post = self::str_repeat($pad_string, (int) ceil($diff / $ps_length));
	...
	*/
	function remove_duplicates($str, $what = ' ') {
		if(is_string($what )) {
			$what = array($what);
		}
		if(is_array($what)) {
	...
	*/
	function trim_util(&$string, &$chrs) {
		if(empty($chrs)) {
			$chrs = self::ws();
		} else if(is_string($chrs)) {
			$chrs = self::split($chrs);
		}
	...
	*/
	function trim($string = '', $chrs = '') {
		$string	= self::ltrim($string, $chrs);
		return self::rtrim($string, $chrs);
	}
	...
	*/
	function ltrim($string = '', $chrs = '') {
		self::trim_util($string, $chrs);
	...
	*/
	function ucwords($str) {
		foreach(self::ws() as $ws) {
			$pos = -1;
			while(true) {
				if(($pos = strpos($str, $ws, $pos + 1)) === false) {
					break;
				}
				$str = self::substr($str, 0, $pos + 1).self::strtoupper(self::substr($str, $pos + 1, 1)).self::substr($str, $pos + 2);
			}
		}
