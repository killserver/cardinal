...
class Defender {
	...
	final public static function readFile($files, $php = false) {
		...
		if(is_readable($files) && $chmod=="0777" && ($fp1 = fopen($files, "r+"))!==false) {
			@chmod($files, 0400);
	...
	final public static function safeSave($file, $d, $php = false) {
		...
		$ret = false;
		if(is_writable($file) && $chmod=="0777" && ($fp = fopen($file, "w+"))!==false) {
			@chmod($file, 0200);
			if($php) {
				$d = '<?php die(); ?>'.$d;
			}
	...
	public static function appendSave($file, $data = "", $php = false) {
		...
		$chmod = substr(sprintf('%o', fileperms($file)), -4);
		$ret = false;
		if(is_writable($file) && $chmod=="0777" && ($fp = fopen($file, "a"))!==false) {
			@chmod($file, 0200);
			if($php) {
				$data = '<?php die(); ?>'.$data;
			}
