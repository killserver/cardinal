	...
	function __construct() {
		...
		if(isset($_GET['out'])) {
			if($level<LEVEL_USER) {
				...
				HTTP::ajax(array("success" => call_user_func_array("User::logout", array())));
			}
		}
		if($level>=LEVEL_USER) {
			location("{C_default_http_local}");
		...
		if(sizeof($_POST)>0) {
			...
			foreach($data as $v) {
				if(!isset($v['applyInLogin'])) continue;
				if(isset($_POST[$v['name']])) {
					$_POST[$v['altName']] = $_POST[$v['name']];
					unset($_POST[$v['name']]);
				}
			}
			HTTP::ajax(array("success" => call_user_func_array("User::login", $_POST)));
		}
		$tmp = "";
		$data = $f['data'];
		$data = execEvent("page_login_data", $data);
		...
		$tpl = execEvent("page_login_tpl", $tpl);
