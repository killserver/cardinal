	...
	function __construct() {
		...
		switch($type) {
			...
			case "search":
				...
				$db = "SELECT {%sel%} FROM {{posts}} WHERE `active` LIKE \"yes\"".(config::Select("new_date") ? " AND `time` <= UNIX_TIMESTAMP()" : "")." AND `type` LIKE \"post\" AND MATCH(`title`, `descr`) AGAINST('+".$string."' IN BOOLEAN MODE) ORDER BY `id` DESC";
			break;
		}
